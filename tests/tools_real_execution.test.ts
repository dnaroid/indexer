...
async function listPlaygroundFiles(): Promise<string[]> {
...
const realDeps = {
  ...
  filterReferences: (results: any[]) => filterReferences(results, PLAYGROUND_DIR, (p: string) => fs.readFile(path.resolve(PLAYGROUND_DIR, p), 'utf8')),
  listProjectFiles: async () => {
    return fg('**/*', { cwd: PLAYGROUND_DIR })
  },
  readFile: (p: string) => fs.readFile(path.resolve(PLAYGROUND_DIR, p), 'utf8'),
...
test('Real Execution: get_file_outline (JS)', async () => {
  ...
  assert.ok(symbols.find((s: any) => s.name === 'UserManager'))
  assert.ok(symbols.find((s: any) => s.name === 'UserManager.addUser'))
  assert.ok(symbols.find((s: any) => s.name === 'init'))
})
...
test('Real Execution: get_file_outline (Python)', async () => {
  ...
  assert.ok(symbols.find((s: any) => s.name === 'Formatter'))
})
...
test('Real Execution: get_file_outline (C#)', async () => {
  ...
  assert.ok(symbols.find((s: any) => s.name === 'Player.Start'))
  assert.ok(symbols.find((s: any) => s.name === 'Player.Attack'))
  assert.ok(symbols.find((s: any) => s.name === 'Player.health')) // serialized field
})
