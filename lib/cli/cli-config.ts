...
export function isInsideIndexer(dir: string): boolean {
  return dir.includes(".indexer")
}

export async function findProjectRoot(startCwd: string): Promise<string> {
  let current = startCwd
  while (current !== "/") {
    const indexPath = path.join(current, ".indexer")
...
export async function loadState(statePath: string): Promise<ProjectState | null> {
  try {
...
export async function saveState(paths: ProjectPaths, patch: Partial<ProjectState>): Promise<ProjectState> {
...
export async function upsertRootMcpConfig(paths: ProjectPaths): Promise<void> {
...
export async function upsertKilocodeMcpConfig(paths: ProjectPaths): Promise<void> {
...
export async function upsertGeminiMcpConfig(paths: ProjectPaths): Promise<void> {
...
export async function upsertCodexArgsConfig(paths: ProjectPaths): Promise<string> {
  const mcpProxyPath = path.join(paths.dotDir, "mcp-server.js")
...
export async function upsertReadmeCliSection(root: string): Promise<void> {
...
export async function ensureGitignoreEntry(root: string, entry: string): Promise<void> {
...
export async function removeRootMcpIndexer(rootMcpPath: string): Promise<void> {
  try {
...
export async function removeKilocodeMcpIndexer(root: string): Promise<void> {
  const targets = ["kilocode/mcp.json", ".kilocode/mcp.json"]
...
export async function removeGeminiMcpIndexer(root: string): Promise<void> {
...
export async function removeCodexArgsIndexer(codexArgsPath: string): Promise<void> {
  try {
...
export async function removeReadmeCliSection(root: string): Promise<void> {
  const readmePath = path.join(root, "README.md")
  let text: string
  try {
    text = await fs.readFile(readmePath, "utf8")
...
export async function removeGitignoreEntry(root: string, entry: string): Promise<void> {
  const gitignorePath = path.join(root, ".gitignore")
  try {
