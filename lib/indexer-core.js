...
const getEmbedModel = () => process.env.EMBED_MODEL || 'nomic-embed-text'
const getVectorSize = () => Number(process.env.VECTOR_SIZE || '768')

/*
  Chunking defaults (tune as needed)
*/
const getMaxChunkLines = () => Number(process.env.MAX_CHUNK_LINES || '220')
const getOverlapLines = () => Number(process.env.OVERLAP_LINES || '30')
...
async function indexFile(relPath) {
  ...
  for (const ch of chunks) {
    ...
    // Skip chunks that are too large for embedding model
    if (!vector) {
      log(`Skipping chunk (too large): ${relPath}:${ch.start_line}-${ch.end_line}`)
      continue
    }
