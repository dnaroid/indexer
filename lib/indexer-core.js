...
async function indexFile(projectRootOrRelPath, relPath, collectionName, settings = {}) {
  ...
  if (lang === 'python') symbols = extractPythonSymbols(content)
  if (lang === 'go') symbols = extractGoSymbols(content)
  if (lang === 'rust') symbols = extractRustSymbols(content)
  if (lang === 'csharp') symbols = extractCSharpSymbols(content)
