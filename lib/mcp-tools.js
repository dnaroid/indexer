...
import {extractJSSymbols, isJSCodeAtPosition} from './ast-js.js'
import {
...
export async function filterReferences(results, workspaceDir, readFile) {
  const filtered = []
  ...
  for (const res of results) {
    ...
    try {
      ...
      } else if (['python', 'go', 'rust', 'csharp'].includes(lang)) {
        isCode = isCodeAtPosition(content, lang, res.line, res.column - 1)
      }
    } catch (e) {
