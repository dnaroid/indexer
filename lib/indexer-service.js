...
// Client tracking for ref-counting
const clients = new Map() // clientId -> {lastHeartbeat, projectPath}
  ...
  heartbeatInterval = setInterval(() => {
    ...
    for (const [clientId, client] of clients) {
      if (now - client.lastHeartbeat > CLIENT_TIMEOUT_MS) {
        log(`Removing dead client: ${clientId}`)
        clients.delete(clientId)
      }
    }
    
    if (clients.size === 0 && inactiveTime >= INACTIVITY_TIMEOUT_MS) {
      log(`No activity and no active clients for ${INACTIVITY_TIMEOUT_MS}ms, shutting down...`)
      console.log(`[indexer-service] Heartbeat check: No clients + inactivity timeout, shutting down...`)
      gracefulShutdown()
    }
...
async function handleRequest(req, res) {
  ...
  if (req.method === 'POST' && req.url === '/heartbeat') {
    ...
    if (clientId) {
      clients.set(clientId, { lastHeartbeat: Date.now() })
    }
    res.writeHead(200, {'Content-Type': 'application/json'})
  ...
  req.on('end', async () => {
    try {
      ...
      if (url.pathname === '/register') {
        ...
        // Only update activity for actual queries (real work)
        updateActivity()
  ...
  return new Promise((resolve, reject) => {
    server.on('error', (e) => {
      if (e.code === 'EADDRINUSE') {
        log(`Port ${SERVER_PORT} already in use - another indexer service may be running`)
        reject(new Error(`Port ${SERVER_PORT} in use`))
      } else {
        reject(e)
      }
    })
...
export async function startIndexerService(projectPathArg = null) {
  ...
  try {
    ...
    if (e.message === `Port ${SERVER_PORT} in use`) {
      console.error(`[indexer-service] Port ${SERVER_PORT} is in use. Another indexer service may be running.`)
      console.error(`[indexer-service] Check with: lsof -i :${SERVER_PORT}`)
      log(`Port ${SERVER_PORT} in use, exiting`)
      process.exit(1)
    }
    throw e
  }
