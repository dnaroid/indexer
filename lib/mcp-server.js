...
async function isServiceUp() {
  try {
    const res = await fetch(`${SERVICE_URL}/health`) // Assuming /health exists or handle 404
    // Actually our simple server returns 404 for health, but connection refutes refused
    // Let's use a known endpoint or just connection check
    return true
  } catch (e) {
    return false
  }
...
async function ensureServiceRunning() {
  ...
  console.error(`[mcp] Starting indexer service: ${servicePath}`)

  const child = spawn('node', [servicePath], {
    detached: true,
    stdio: 'ignore', // Log file handles output
    env: process.env
  })
...
function createProxyHandlers(projectPath) {
  return {
    search_codebase: (args) => queryService(projectPath, 'search_codebase', args),
    search_symbols: (args) => queryService(projectPath, 'search_symbols', args),
    get_file_outline: (args) => queryService(projectPath, 'get_file_outline', args),
    get_project_structure: (args) => queryService(projectPath, 'get_project_structure', args),
    find_usages: (args) => queryService(projectPath, 'find_usages', args)
  }
}
...
export async function startMcpServer(startCwd, args, projectPathArg = null) {
  ...
  await startIndexerService(projectPathArg)
