...
export async function handleStartDaemon(startCwd) {
  if (await isDaemonRunning(startCwd)) {
    log('Indexer daemon is already running.')
    return
  }

  log('Starting indexer daemon...')
  const daemonCommand = `nohup node ${path.resolve(startCwd, 'indexer.js')} mcp --project=${startCwd} > /dev/null 2>&1 &`

  spawn('bash', ['-c', daemonCommand], {
    cwd: startCwd,
    detached: true,
    stdio: 'ignore'
  }).unref()
  ...
  if (await isDaemonRunning(startCwd)) {
    log('Indexer daemon started successfully.')
  } else {
    fail('Failed to start indexer daemon.')
  }
}
