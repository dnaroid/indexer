...
export async function checkAndAutoUpdate(command) {
  ...
  try {
    ...
    if (latest && latest !== currentVersion) {
      console.log(`\n\x1b[33m[!] New version available: ${latest} (current: ${currentVersion})\x1b[0m`)
      if (!command) {
        if (await confirmAction('    Would you like to update now?')) {
          await handleUpdate()
          console.log('\x1b[32mPlease restart the tool to use the new version.\x1b[0m')
          process.exit(0)
        }
      }
    }
