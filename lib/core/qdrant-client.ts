...
let fetch: any = globalThis.fetch
...
interface QdrantPointsScrollResponse {
  result?: {
    points: any[]
...
async function ensureCollections(
  ...
  const collections: QdrantCollectionsResponse = await qdrantRequest('/collections', null, 'GET', settings)
 ...
 * @param {IndexerSettings} settings - Settings with QDRANT_URL
 */
async function dropCollection(collectionName: string, settings: IndexerSettings = {}): Promise<void> {
 ...
 * @param {IndexerSettings} settings - Settings with QDRANT_URL
 */
async function createPayloadIndexes(collectionName: string, settings: IndexerSettings = {}): Promise<void> {
  const indexes = [
 ...
 * @param {IndexerSettings} settings - Settings with QDRANT_URL
 */
async function upsertPoints(collectionName: string, points: any[], settings: IndexerSettings = {}): Promise<void> {
 ...
 * @param {IndexerSettings} settings - Settings with QDRANT_URL
 */
async function deletePointsByPath(collectionName: string, filePath: string, settings: IndexerSettings = {}): Promise<void> {
 ...
 */
async function getExistingFileHash(
  collectionName: string,
  filePath: string,
  settings: IndexerSettings = {}
): Promise<string | null> {
  const res: QdrantPointsScrollResponse = await qdrantRequest(`/collections/${collectionName}/points/scroll`, {
    filter: {
      must: [{key: 'path', match: {value: filePath}}]
    },
    limit: 1,
 ...
 */
async function listAllIndexedPaths(collectionName: string, settings: IndexerSettings = {}): Promise<Set<string>> {
  let offset: any = null
  const paths = new Set<string>()
  while (true) {
    const body: ScrollBody = { limit: 1000, with_payload: ['path'], with_vector: false }
    if (offset) body.offset = offset
    const res: QdrantPointsScrollResponse = await qdrantRequest(`/collections/${collectionName}/points/scroll`, body, 'POST', settings)
 ...
 */
async function listCollectionsExtended(settings: IndexerSettings = {}): Promise<Array<{name: string, count: number}>> {
  const data: QdrantCollectionsResponse = await qdrantRequest('/collections', null, 'GET', settings)
  const collections = data.result?.collections || []
  const results: Array<{name: string, count: number}> = []
  for (const c of collections) {
    let count = 0
    try {
      const res: QdrantCountResponse = await qdrantRequest(`/collections/${c.name}/points/count`, {exact: true}, 'POST', settings)
