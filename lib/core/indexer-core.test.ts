...
interface TestRecord {
  ...
  chunkPoints: any[] | null
  metaPoints: any[] | null
}

function createFetchMock(record: TestRecord): typeof globalThis.fetch {
  return async function fetch(url: string, options: RequestInit = {}) {
...
function assertSetEqual(actual: string[], expected: string[], message: string): void {
...
interface LanguageTestCase {
  ...
  expectedUnityTags?: string[]
  unexpectedNames?: string[]
}

async function setupLanguageTest({ ext }: { ext: string }): Promise<{ relPath: string; record: TestRecord }> {
  ...
  const record: TestRecord = { fetchCalls: [], chunkPoints: null, metaPoints: null }
...
async function runLanguageCase({ ext, lang, expectedNames, expectedKinds, expectedUnityTags, unexpectedNames }: LanguageTestCase): Promise<void> {
