 ...
 */
export async function isQdrantUp(url: string): Promise<boolean> {
  try {
 ...
 */
export async function isOllamaUp(url: string): Promise<boolean> {
  try {
 ...
 */
export async function countIndexed(collectionName: string): Promise<number | null> {
  ...
  try {
    ...
    const json: QdrantCountResponse = await res.json()
 ...
 */
export async function handleListCollections(silent: boolean = false): Promise<Array<{name: string, count: number}>> {
 ...
 */
export async function handleDeleteCollection(name?: string): Promise<void> {
  await ensureQdrantConnection()
  ...
  if (!name) {
    ...
    let toDelete: Array<{name: string, count: number}> = []
    if (trimmed === 'all') {
      toDelete = list
    } else {
 ...
 * @returns {Promise<void>}
 */
export async function handlePruneAll(): Promise<void> {
  await ensureQdrantConnection()
  const list = await handleListCollections(true)
  if (list.length === 0) {
