...
const FIXTURES_DIR = path.resolve(process.cwd(), 'lib/test_fixtures')
...
test('Go parsing captures functions, methods, and types', async () => {
  await runLanguageCase({
    ext: '.go',
    lang: 'go',
    expectedNames: ['run', 'DoWork', 'User', 'Notifier'],
    expectedKinds: ['function', 'method', 'type']
  })
})

test('Rust parsing captures functions, structs, and impl methods', async () => {
  await runLanguageCase({
    ext: '.rs',
    lang: 'rust',
    expectedNames: ['do_stuff', 'Widget', 'Widget.render', 'render'],
    expectedKinds: ['function', 'struct', 'method']
  })
})
