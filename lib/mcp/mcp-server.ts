 ...
 * - indexer-service.js with --mcp flag for global MCP server
 * - .indexer/mcp-server.js proxy script (generated during init)
 *
...
export const CODEBASE_PROMPT = [
  ...
  '- Use find_usages to ensure changes don\'t break other parts of system.',
  '',
  'Unity Specifics:',
  '- Lifecycle methods (Awake, Start, Update) are called by engine, not explicitly in code.',
...
async function isServiceUp(): Promise<boolean> {
  try {
    const res = await fetch(`${SERVICE_URL}/health`, { signal: AbortSignal.timeout ? AbortSignal.timeout(2000) : undefined })
    if (res.ok) {
...
async function queryService(projectPath: string, tool: string, args: any): Promise<any> {
...
function createProxyHandlers(projectPath: string) {
  return {
...
export async function startMcpServer(startCwd: string, args: any, projectPathArg: string | null = null): Promise<void> {
  log(`Starting MCP server from ${startCwd} for project ${projectPathArg || 'all registered'}...`)
  await initTreeSitter()

  // Load service port from config
  const {loadGlobalConfig, getProjectConfig, DEFAULT_SETTINGS} = await import('../utils/config-global.js')
  ...
  // Register project, if specified
  if (projectPathArg) {
    await registerProject(projectPathArg)
  }

  // Create and start MCP server via stdio
